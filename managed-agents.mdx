...
Sign up at [mengram.io](https://mengram.io/#signup) — free tier includes 30 adds and 100 searches per month.
...
Managed Agents use [vaults](https://docs.anthropic.com/en/docs/agents/managed-agents#vaults) for secrets. Create a vault with your Mengram API key and pass it when creating a session:
...
# Create session with vault containing MENGRAM_API_KEY
session = client.agents.sessions.create(
    agent_id=agent.id,
...
```json
{
  "type": "url",
  "name": "mengram",
  "url": "http://your-host:8420/mcp/sse"
}
```
