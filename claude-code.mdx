...
**Why the record fills itself here and not offline.** The gate is only as good as the counts behind it. With the cloud, auto-save extracts workflows from your sessions, a failed run records the violated assumption on a new version, and every procedure carries `success_count` / `fail_count` / `last_failure` without you writing anything. The same gate runs fully offline against a [memfmt](https://github.com/alibaizhanov/memfmt) folder (`MENGRAM_MEMORY_DIR=./memory`), but there the counts are whatever you typed.
