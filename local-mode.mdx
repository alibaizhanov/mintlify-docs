...
```bash
mengram import claude-code --memory ./memory            # 20 most recent sessions
mengram import claude-code --memory ./memory --last 50 --project shop
```

Each session becomes one extraction with the folder's model, redacted for keys and tokens before it is read; nothing leaves your machine except those model calls. The import ends with what the folder now holds and the workflows it learned, with their record. Imported sessions are listed in `.mengram/claude-code-imported.json`, so re-runs pick up only new ones (`--reimport` forces).
