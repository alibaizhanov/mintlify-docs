...
```bash
curl -X DELETE "https://mengram.io/v1/account?confirm=you@example.com" \
  -H "Authorization: Bearer om-..."
```

| Parameter | Type | Description |
|-----------|------|-------------|
| `confirm` | string | **Required.** Must exactly match your account email |
...
| `max_tokens` | int | null | Room the reply may take, in estimated tokens (50–50,000). Results are cut in rank order to fit — entity by entity, fact by fact — and the response adds a `budget` report. |

**A recall that fits.** With `max_tokens`, the response carries what was cut, so the cut is never silent:

```json
{"query": "what does this customer prefer?", "user_id": "cust_1042", "limit": 5, "max_tokens": 600}
```
...
```json
{
  "results": [...],
  "budget": {
    "max_tokens": 600, "used_tokens": 512, "method": "estimate",
    "kept":    {"entities": 3, "facts": 14},
    "dropped": {"entities": 2, "facts": 9, "relations": 1, "knowledge": 2}
  }
}
```

Tokens are estimated (about four characters per token for Latin text, fewer for other scripts) — the server does not know which model reads the reply. Budget around 15% under your real limit.

**Where each fact came from.** Every result carries `facts_meta`, one entry per fact, aligned with `facts`:

```json
{
  "entity": "app", "type": "project", "score": 0.91,
  "facts": ["deploys to Fly.io from main", "uses PostgreSQL 16"],
  "facts_meta": [
    {"source": "codex", "os": "linux", "cwd": "/home/ali/app", "session": "s-41",
     "when": "2026-09-15", "recalled": 3, "last_recalled": "2026-09-16"},
    {"source": "api", "when": "2026-09-10", "recalled": 0}
  ]
}
```

`source` is what the writer declared on `/v1/add` (`source` and `metadata`); the CLI hooks send `claude-code`, `codex` or `cursor` with the session, working directory and OS, the MCP server sends `mcp`, everything else is `api`. `recalled` and `last_recalled` count retrievals, so a fact nobody has asked for in months is visible as such. Facts are never rewritten; the tag the hooks show (`(codex, 2026-09-15)`) is built from this at read time.

**A host does not inherit another host's paths.** Send `X-Mengram-Host: <os>/<tool>` (for example `linux/claude-code`; the hooks do) and facts tied to another machine or tool are left out: a `/Users/...` path recorded on a Mac, a `~/.cursor/hooks.json` fact recorded from Cursor. Each result then says how many were withheld in `facts_left_out_for_host`. Without the header nothing is filtered, and facts saved without provenance are never filtered. From Python: `CloudMemory(api_key, host="linux/claude-code")`.
...
```json
{"query": "deployment", "user_id": "default", "limit": 5, "max_tokens": 800}
```

`max_tokens` works here too: one budget across the sections, spent in the order a task needs them — facts about the entities, then what happened, then how it was done, then raw conversation last. The `budget` report also counts `episodes`, `procedures` and `chunks`.

`chunks` (default `1`, `0` to `10`) is how many raw conversation chunks may come back as the fallback for facts extraction missed. One is the default because on a 90-day history five of them were three quarters of the tokens handed to the model while one kept recall of old facts unchanged; `0` turns the fallback off.
...
<Tip>**Pro / Growth / Business only.** Free and Starter plans return 403.</Tip>
...
List recent episodes. Each episode has a summary, context, outcome, and participants. Paginated — the response includes `total`, `limit`, and `offset` so you can page through everything.
...
List learned workflows and procedures. Paginated — the response includes `total`, `limit`, and `offset`.
...
Record success or failure for a procedure. On failure with context, triggers experience-driven evolution (Pro and above).

This is not the only way a procedure evolves. Mengram also infers failures from episodes as they are added and revises the matching procedure on its own, which in practice is the more common path. A revision only earns a new version number once the current one has succeeded at least once — a version nobody ran is a hypothesis, so it is edited in place rather than superseded.
...
Failure-driven revisions record the **violated assumption** (the specific belief that turned out false — not just the step number) in the evolution history, and derive a **precondition check** that accumulates in the procedure's `metadata.preconditions` — so recall returns not just the steps but what to verify before trusting them.
...
```bash
curl -X POST "https://mengram.io/v1/identity?entity=YourName" \
  -H "Authorization: Bearer om-..."
```

| Parameter | Type | Default | Description |
|-----------|------|---------|-------------|
| `entity` | string | required | Exact name of the entity that represents you |
| `user_id` | string | `"default"` | Sub-user scope |

Returns `{"status": "pinned", "entity": "...", "entity_id": "..."}`. Pinning clears the flag from any previously pinned entity. 404 if the entity doesn't exist.
...
Get all reflections. Optional `scope` filter: `entity`, `cross`, `temporal`. Each item includes its `id` for individual deletion.

### DELETE /v1/reflections/:id

Delete a single reflection by id. Use when a generated reflection is wrong or polluted — the next reflection pass regenerates from clean facts.

```bash
curl -X DELETE "https://mengram.io/v1/reflections/<id>" \
  -H "Authorization: Bearer om-..."
```

Returns `{"status": "deleted", "reflection_id": "..."}`, or 404 if the id doesn't exist for this user.
...
Webhook adapters that turn Vapi's tool-call and end-of-call events into Mengram memory operations. See the full guide at [Vapi Integration](/vapi).

### POST /v1/voice/vapi/recall

Called by Vapi as a custom tool. Returns a concise context string the assistant verbalizes at the start of a call.

**Request body** (Vapi sends both shapes; either accepted):
...
Or the OpenAI-nested form (`toolCalls` with `function.name` + `function.arguments` — the latter may arrive as a JSON string):
...
**Phone resolution order:** tool arguments → `message.call.customer.number` → "Unknown caller" fallback.
...
```json
{
  "results": [{
    "toolCallId": "call_abc",
    "result": "Known about caller (Sarah Johnson): Sarah Johnson: prefers morning slots before 11 AM | Sarah Johnson: gets anxiety with novocaine"
  }]
}
```

If `message.type` is anything other than `tool-calls` (Vapi sends many event types to the same server URL), the endpoint returns `200 {"status":"ignored","reason":"..."}` rather than 4xx — so the assistant isn't marked broken when lifecycle events arrive.

**Quota:** 1 search per call.

### POST /v1/voice/vapi/save

Called by Vapi at end of call. Routes the final transcript through Mengram's extraction pipeline, keyed per caller via `sub_user_id=voice:<E.164>`.
...
The transcript can also live at `message.artifact.transcript` — Mengram reads whichever is present.
...
**Only `end-of-call-report` triggers extraction.** Partial `transcript` events (Vapi streams these mid-call) and other lifecycle events are ignored with `{"status":"ignored"}` so the extraction pipeline doesn't fire repeatedly per call.
...
SDKs expose quota headers via the `.quota` property — see [Python SDK](/python-sdk#quota-usage) and [JavaScript SDK](/javascript-sdk#quota-usage).
