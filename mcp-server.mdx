...
| `context_for` | Get relevant memory context for a specific task. Returns a compact context pack: related entities, procedures, and past events. Takes `max_tokens` (default 1200): the pack is cut in rank order to fit and ends with one line naming what was left out. |
