...
Where the task in this repository stands. The hooks write a task card when an agent stops — keyed by `git remote` + branch, so another session, another agent in another worktree or another machine finds it — and inject it at the next session start, saying whether the code has moved since the last check.
